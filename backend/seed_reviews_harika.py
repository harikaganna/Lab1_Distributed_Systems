"""Seed reviews for all restaurants from harika.ganna@gmail.com"""
...
REVIEWER_EMAIL = "harika.ganna@gmail.com"
...
with engine.connect() as conn:
    row = conn.execute(text("SELECT id FROM users WHERE email = :e"), {"e": REVIEWER_EMAIL}).fetchone()
    if not row:
    ...
    for rid, rname in restaurants:
        ...
        comment = random.choice(REVIEWS[rating])
        conn.execute(
            text("INSERT INTO reviews (rating, comment, user_id, restaurant_id) VALUES (:rating, :comment, :uid, :rid)"),
...
print(f"Done! Added {added} reviews for '{REVIEWER_EMAIL}' ({len(restaurants) - added} already existed).")
