...
export default function EditRestaurant() {
  ...
  useEffect(() => {
    restaurantApi.get(`/restaurants/${id}`)
      .then((res) => {
        const r = res.data;
        setForm({
          name: r.name, cuisine_type: r.cuisine_type, city: r.city,
          description: r.description || "", address: r.address || "", state: r.state || "",
          zip_code: r.zip_code || "", phone: r.phone || "", price_range: r.price_range || "",
          ambiance: r.ambiance || "", hours: r.hours || "", amenities: r.amenities || "",
        });
      })
  ...
  async function handleSubmit(e) {
    ...
    try { await restaurantApi.put(`/restaurants/${id}`, payload); setSuccessMsg("Restaurant updated!"); }
    catch (err) { setError(err.response?.data?.detail || "Error updating restaurant"); }
  }
    ...
    <div className="row justify-content-center">
      <div className="col-md-8">
        <div className="card card-clean">
          <div className="card-header"><h5 className="mb-0">Edit Restaurant</h5></div>
          <div className="card-body">
            {error && <div className="alert alert-danger alert-clean py-2">{error}</div>}
            {successMsg && <div className="alert alert-success alert-clean py-2">{successMsg}</div>}
            <form onSubmit={handleSubmit}>
              <div className="row g-3">
                ...
                <div className="col-md-2"><label className="form-label">State</label><input className="form-control" value={form.state} onChange={updateField("state")} maxLength={5} /></div>
                ...
                <div className="col-md-6"><label className="form-label">Hours</label><input className="form-control" value={form.hours} onChange={updateField("hours")} /></div>
                <div className="col-md-6"><label className="form-label">Amenities</label><input className="form-control" value={form.amenities} onChange={updateField("amenities")} /></div>
              </div>
              <button className="btn btn-brand mt-3" type="submit">Save Changes</button>
            </form>
          </div>
        </div>
      </div>
